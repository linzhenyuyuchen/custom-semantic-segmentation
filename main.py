...
############################################
import model as M
import dataset.wsiDataset as D
...
def validate(loader, net, criterion, device, validate_args, global_step=0):
    ...
    with tqdm(total=n_val, desc='Validation round', unit='subject', leave=False) as pbar:
        for batch in loader:
            total_step += 1
            imgs = batch["image"]  # (1,m,c,h,w)
            masks_onehot = batch['mask'][0]  # (m,h,w)
            imgs = imgs.to(device=device, dtype=torch.float32)  # [B,c,H,W]
            # masks_onehot = masks_onehot.to(device=device, dtype=torch.long) # [B,1,H,W]
            masks_onehot = make_one_hot(masks_onehot, n_classes)  # (B,n_classes,H,W)
            # gts_all.append(masks_onehot)
            ############################################
            with torch.no_grad():
                masks_pred = np.zeros((imgs.shape[1], 1, imgs.shape[3], imgs.shape[4]))
                for i in range(imgs.shape[1]):
                    masks_pred[i] = torch.argmax(net(imgs[:, i]).squeeze(0), dim=0).cpu().numpy()
            ...
            x1 = vutils.make_grid(masks_onehot.float().cpu(), normalize=True, scale_each=True)
            x2 = vutils.make_grid(masks_pred.float().cpu(), normalize=True, scale_each=True)
...
if __name__ == '__main__':
    ...
    cfgs = yaml.load(open("./config/config.yaml", "r"), Loader=yaml.FullLoader)
    args = get_args()
