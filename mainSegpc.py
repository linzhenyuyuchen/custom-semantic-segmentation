...
def validate(loader, net, criterion, device, validate_args, global_step=0):
    ...
    with tqdm(total=n_val, desc='Validation round', unit='subject', leave=False) as pbar:
        for batch in loader:
            ...
            masks_onehot = batch['mask']  # (1,h,w)
            imgs = imgs.to(device=device, dtype=torch.float32)
            masks_onehot = masks_onehot.to(device=device, dtype=torch.long)
            masks_onehot = make_one_hot(masks_onehot, n_classes)  # (1,n_classes,H,W)
            ############################################
            with torch.no_grad():
                masks_pred = net(imgs)  # (1,n_classes,H,W)
            masks_pred = torch.argmax(masks_pred, dim=1)# (1,H,W)
            masks_pred = make_one_hot(masks_pred, n_classes)  # (1,n_classes,H,W)

            # x1 = vutils.make_grid(masks_onehot, normalize=True, scale_each=True)
            # x2 = vutils.make_grid(masks_pred, normalize=True, scale_each=True)
            ...
            # print(masks_onehot.shape)
            masks_pred = masks_pred.float()[0]
            masks_onehot = masks_onehot.float()[0]
            for c in range(n_classes):
                if torch.max(masks_onehot[c]) > 0:
                    count[c] += 1
